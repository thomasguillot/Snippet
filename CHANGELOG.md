...
- **Download & save dialog:** After successful conversion, a “Show in Finder” button appears below the card and opens the saved file. The main process now uses a Save dialog (defaulting to Downloads) so users can choose where to save the MP3, and it returns the chosen file path; canceling shows a clear “Save canceled. Your MP3 was not saved.” message instead of a raw IPC error.
