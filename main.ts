...
function getAllowedBasePaths(): string[] {
	...
	const dirs: string[] = [path.resolve(home)];
...
function getLocalFileDuration(filePath: string): Promise<number | null> {
	return new Promise((resolve) => {
		if (!ffmpegPath) {
			resolve(null);
			return;
		}
		function parseStderr(stderr: string): number | null {
...
ipcMain.handle('open-file-dialog', async (event: IpcMainInvokeEvent) => {
	...
	const win =
		(event.sender as unknown as { getOwnerBrowserWindow?: () => BW | null }).getOwnerBrowserWindow?.() ??
		mainWindow;
...
ipcMain.handle('get-local-file-info', async (event: IpcMainInvokeEvent, { filePath }: { filePath: string }) => {
	if (!isAllowedSender(event)) {
		throw new Error('Unauthorized');
	}
